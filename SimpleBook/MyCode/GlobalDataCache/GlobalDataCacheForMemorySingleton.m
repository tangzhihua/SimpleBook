...
#pragma mark -
#pragma mark 因为 GlobalDataCacheForNeedSaveToFileSystem 要对 localBookList 属性进行KVO 监听, 所以这里要始终保证 localBookList 属性始终有效并且不变.
@synthesize localBookList = _localBookList;
-(LocalBookList *)localBookList {
  if (_localBookList == nil) {
    _localBookList = [[LocalBookList alloc] init];
  }
  
  return _localBookList;
}

-(void)setLocalBookList:(LocalBookList *)newLocalBookList{
  if (![newLocalBookList isKindOfClass:[newLocalBookList class]]) {
    return;
  }
  
  for (LocalBook *book in newLocalBookList.localBookList) {
